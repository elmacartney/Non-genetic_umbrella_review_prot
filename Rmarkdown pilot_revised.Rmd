---
title: "Non-genetic inheritance of environmental exposures: a protocol for a map of systematic reviews with bibliometric analysis - pilot code"
...
Manually extracted data used to address objectives 1,2 and 4 are stored in an .xls file with multiple spreadsheets within the same file. The tabs ending with "_code_info" do not have extracted data but have descriptions of each data element (presented in tables under each relevant subheading). Bibliometric data used to address object 3 is stored as a .bib file that was exported directly from Scopus. Both files can be found on the open Science Framework https://osf.io/detvk/ 
...
Splitting list of dataframes into separate dataframes and joining controlled vocabulary dataframes with coded data.
...
Note that the description often equals controlled vocab but this is not always the case for more complicated elements.
``` {r, }
Descendant_generat_info_unique <- Descendant_generat_code_info %>% 
  select(
    controlled_vocab_descendant_generat, description
  )
unique(Descendant_generat_info_unique) 
```
