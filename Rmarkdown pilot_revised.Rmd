...
# 
# #Env_eff_direction
# Env_eff_direction_info <- left_join(Env_eff_diection_info, Env_eff_direction_code_info)
# 
...
count_descendant_sex <- merged_descendant_sex %>% count(controlled_vocab_descendant_sex, by = discipline_code) %>% arrange(desc(n))
...
count_descendant_age <- merged_descendant_age %>% count(controlled_vocab_descendant_age, by = discipline_code) %>% arrange(desc(n))
...
count_higher_taxon <- Merged_higher_taxon %>% count(controlled_vocab_taxon, by = discipline_code) %>% arrange(desc(n))
